...
app.post('/login', (req, res) => {
  ...
  app.post('/booking', (req, res) => {
    const { id, moviename, name, banyakTiket, pilihKursi } = req.body;
  
    // Lakukan verifikasi terhadap nilai id di sini, misalnya:
    db.query("SELECT id FROM login WHERE id = ?", [id], (err, result) => {
      if (err) {
        console.error("Error executing SQL query:", err);
        return res.status(500).json({ error: "An error occurred" });
      }
      ...
      // Jika id valid, lakukan INSERT ke dalam tabel booking
      const sql = "INSERT INTO booking (id, moviename, name, banyakTiket, pilihKursi) VALUES (?, ?, ?, ?, ?)";
      const values = [id, moviename, name, banyakTiket, pilihKursi];
