...
app.delete('/history', (req, res) => {
    ...
    // Lakukan penghapusan data booking berdasarkan nama di database
    // ...
    // Respon dengan status dan pesan yang sesuai
...
app.get('/api/currentUser', (req, res) => {
  ...
  async function run (){
    parser.User.enableUnsafeCurrentUser();
    try{
        await parser.User.login("alex","1234");
        const currentUser = parser.User.current();
    }if{
        console.log("User :" + currentUser.get("username"));

    }else{
        console.lo{g("user tidak ditemukan");
    }catchs(ex){
        console.logS("user login");
    }
