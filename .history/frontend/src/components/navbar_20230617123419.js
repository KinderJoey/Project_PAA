...
const Navigation = () => {
  ...
  function App() {
    ...
    const checkLoginStatus = () => {
      // Lakukan permintaan ke endpoint backend untuk memeriksa status login
      axios.get('/check-login')
        .then((response) => {
          setIsLoggedIn(response.data.isLoggedIn); // Set status login berdasarkan respons dari server
        })
        .catch((error) => {
          console.error('Terjadi kesalahan:', error);
        });
