...
const Triller = () => {
  ...
  return (
    <div className="triller_body">
      <header className="center-max-size header">
        <span className={"brand-triller"}>d<b>MOVIE</b></span>
        <form className="form-triller" onSubmit={fetchMovies}>
      ...
      {movies.length ?
        <main>
          {movie ?
            <div className="poster-triller" style={{ backgroundImage: `linear-gradient(to bottom, rgba(0, 0, 0, 0), rgba(0, 0, 0, 1)), url(${BACKDROP_PATH}${movie.backdrop_path})` }}>
              {playing ?
                <> 
                  <Youtube
                    videoId={trailer.key}
                    className={"youtube amru"}
                    container-triller ClassName={"youtube-container amru"}
                    opts={{
          ...
          <div className={"center-max-size container-triller"}>
            {renderMovies()}
          </div>
        </main>
